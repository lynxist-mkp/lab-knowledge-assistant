...
对外契约需要明确三个相关但不同的概念：**知识库**（系统能力）、**集合**（材料作用域容器）、**文化域**（集合内的内容切分维度）。此前 `settings.product.collection` 已隐式界定索引与配图边界，但缺少一等读模型与稳定对外命名，导致 HTTP、MCP 与运维观测各自拼装 scope。
...
3. **文化域** 是集合内的内容维度，用于浏览、过滤与提问可选范围；不等同于集合，也不作为集合 ID。
...
- 术语表（`CONTEXT.md`）中的 **集合** 与 **文化域** 边界在代码中有对应类型，而非仅配置注释。
