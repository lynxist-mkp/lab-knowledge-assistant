...
**Updated:** [#29 评测单次检索](https://github.com/lynxist-mkp/wenmai-assistant/issues/29)
...
1. **评测单次检索、单次生成**：`eval/pipeline.py` 的 `eval_item()` 对每条 golden × 每组先调用一次 `retrieve()`，再对同一批 chunks 调用 `wenmai.generation.generate`（不经过 `ask_question`）。
   - **Hit@5 / MRR**：由检索结果计算。
   - **拒答准确率 / 出处覆盖率**：由生成结果得到 `refused` 与 `citations`。
   - **零 chunks**：仍调用 `generate()`，其内部短路为 `insufficient_evidence` 拒答，不调用 LLM。
2. **评测全程不写 query trace**：评测不调用 `save_trace`；编辑工作台与 `/ask` 默认行为不变（仍写 trace）。
