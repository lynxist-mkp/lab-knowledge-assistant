...
黄金集评测会对每条标注题、每个消融组反复跑完整提问链路。原先评测经 `ask_question()`，在 `finally` 里无条件 `save_trace`，导致运维追踪里混入大量评测产生的 query trace，污染真实检索工作台提问的观测数据，也让 trace 文件体积无意义膨胀。
...
2. **检索工作台与 `/ask` 默认写 trace**：`ask_question` → `run_ask_works` 在 `record_trace=True` 时经 `QueryTrace` 落盘。
